...
# Function to analyze JD and Resume match
...
5. Soft Skills Match
6. Overall Match Percentage between 1 t0 100, give exact percentage
...
def generate_interview_questions(job_description, resume):
    prompt = f"Job Description:\n{job_description}\n\nCandidate Resume:\n{resume}\n\nGenerate interview questions based on the job description and candidate resume."
    response = llm._call(prompt,user)
    return response['data']['content'] # type: ignore
 
# Function to simulate interview and analyze skills
...
if "allow_interview" not in st.session_state:
...
if job_description_file and resume_file and job_description and resume:
        ...
        if match_score < 60:
            ...
            with col2:
                if st.button("Skip This Role"):
                    st.session_state.allow_interview = False
                    st.session_state.start_interview = False
                    st.info("You chose to skip this role.")
        else:
