...
You are a professional resume analyst. Extract the following structured information from the resume text provided below.
...
def read_resume_with_langchain(file_path):
    try:
        loader = PyMuPDFLoader(file_path)
        documents = loader.load()
        return "\n".join([doc.page_content for doc in documents])
    except Exception as e:
        st.error(f"Error reading PDF with PyMuPDFLoader: {e}")
        return None
    ...
    try:
        if uploaded_file.type == "text/plain":
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
                ...
                tmp_path = tmp.name
            return read_resume_with_langchain(tmp_path)
...
def parse_resume(resume_text):
    try:
        summarised_response = llm._call(
            "You are a professional resume analyst. Extract the following structured information from the resume text provided below.Kindly provide summary of profile, ensuring to include full name, email address, phone number, only single list of technical skills without categorizing, details of business capabilities, an overview of functional capabilities and complete professional experience along with roles and responsibilities, special notes read the entire profile before summarising as there can be multiple profile formats embedded one below another. Read profile till the end and then summarise." + resume_text,
            user="user"
        )
        ...
        # try:
        #     validated_resume = Resume(**parsed_resume)
...
if uploaded_files:
    ...
    for idx, uploaded_file in enumerate(uploaded_files):
        with st.spinner(f"Processing {uploaded_file.name}..."):
            try:
                resume_text = read_resume_with_langchain(r"C:\Users\MA20306913\Downloads\Resume_Ankit_Lohiya.pdf")
                if not resume_text:
...
images = [
    ("Kallisti", r"C:\Users\MA20306913\Desktop\New folder\\Layout1.png", option_one),
    ("Phaedon", r"C:\Users\MA20306913\Desktop\New folder\\Layout2.png", option_two),
    ("Erasmos", r"C:\Users\MA20306913\Desktop\New folder\\Layout3.png", option_three),
]
...
# Unified download section (ZIP even for one resume)
if parsed_results:
    layout_options = {
    ...
    st.markdown("<h8 style='font-size: 16px;color:#17365D;'> 📦 Download Resume(s) as ZIP</h8>", unsafe_allow_html=True)
    zip_file_path = generate_and_zip_resumes(parsed_results, selected_layout_function)
 
    if zip_file_path:
        with open(zip_file_path, "rb") as f:
            st.download_button(
                label="⬇️ Download Resume(s)",
                data=f.read(),
